# -*- coding: utf-8 -*-
"""
Created on Wed Jan 20 10:57:56 2016
Training_objective for LGCPP
@author: user
"""
...
def theano_funcs(energy, stats_dict):
    ...
    ndim = initial_pos.shape[1].astype(theano.config.floatX)
    ...
    f_df      = theano.function([initial_pos, energy.Y, initial_vel,decay_rates, stepsizes,n_step], report_scalars+gparams, name='func_f_df',updates = updates,  allow_input_downcast=True)
    f_samples = theano.function([initial_pos, energy.Y, initial_vel,decay_rates, stepsizes,n_step], [all_pos, all_vel, all_noise, final_pos], name='func_samples', updates = updates, allow_input_downcast=True)
...
class training_objective:
    ...
    def f_df(self, params, *args):
       ...
       # print "length of args", len(args)
        ...
        stepsizes0 = args[3]
        n_step = args[4]
        theano_args = [params] + [energy_Y, initial_v, decay, stepsizes0, n_step]
